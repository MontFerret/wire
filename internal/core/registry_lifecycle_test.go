...
func TestConnectionCloseIsIdempotentAndRejectsNewResources(t *testing.T) {
	...
	if _, err := connection.Compile(context.Background(), CompileInput{Source: api.Source{Content: "RETURN 2"}}); !hasCategory(err, ErrorInvalidState) {
		t.Fatalf("compile was accepted while connection was closing: %v", err)
	}

	if _, err := connection.Execute(context.Background(), ExecuteInput{PlanID: compiled.ID}); !hasCategory(err, ErrorInvalidState) {
		t.Fatalf("execution was accepted while connection was closing: %v", err)
	}

	if _, err := connection.OpenDebugSession(context.Background(), OpenDebugInput{PlanID: compiled.ID}); !hasCategory(err, ErrorInvalidState) {
...
func TestPlanReleaseWaitsForInFlightDebugCreation(t *testing.T) {
	...
	if _, err := connection.Execute(context.Background(), ExecuteInput{PlanID: compiled.ID}); !hasCategory(err, ErrorPlanNotFound) {
		t.Fatalf("closing plan accepted an execution: %v", err)
	}
	if _, err := connection.OpenDebugSession(context.Background(), OpenDebugInput{PlanID: compiled.ID}); !hasCategory(err, ErrorPlanNotFound) {
	...
	if err := <-openResult; !hasCategory(err, ErrorPlanNotFound) {
...
func TestExecutionTerminalStateSurvivesCancellationOrdering(t *testing.T) {
	t.Run("cancellation wins before completion", func(t *testing.T) {
		...
		if settled.State != ExecutionCancelled {
	...
	t.Run("completion remains terminal after cancellation", func(t *testing.T) {
		...
		settled := waitExecution(t, connection, started.ID)
		if settled.State != ExecutionCompleted {
		...
		if afterCancel.State != ExecutionCompleted {
