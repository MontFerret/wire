...
func TestEventStreamPublishesMonotonicEventsAndReplaysLatest(t *testing.T) {
	stream := newEventStream[streamTestEvent](2)
...
func TestEventStreamUnsubscribeIsIdempotentAndReleasesCapacity(t *testing.T) {
	stream := newEventStream[streamTestEvent](1)
...
func TestEventStreamTerminalPublishAndClose(t *testing.T) {
	...
	closed := newEventStream[streamTestEvent](1)
...
func TestEventStreamSupportsConcurrentPublishSubscribeAndCancel(t *testing.T) {
	...
	stream := newEventStream[streamTestEvent](operations)
...
func TestResourceWatchersRetainTheirLimitErrors(t *testing.T) {
	execution := &Execution{events: newEventStream[ExecutionEvent](1)}
	...
	if _, err := execution.Watch(); !hasCategory(err, ErrorResourceExhausted) || err.Error() != "execution watcher limit reached" {
		t.Fatalf("unexpected execution watcher limit error: %v", err)
	}

	session := &DebugSession{events: newEventStream[DebugEvent](1)}
	...
	if _, err := session.Watch(); !hasCategory(err, ErrorResourceExhausted) || err.Error() != "debug watcher limit reached" {
