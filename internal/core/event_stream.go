...
type (
	streamEvent[T any] interface {
		clone() T
		withSequence(uint64) T
	}

	eventStream[T streamEvent[T]] struct {
		mu               sync.Mutex
...
func newEventStream[T streamEvent[T]](maxSubscriptions int) *eventStream[T] {
...
func (s *eventStream[T]) publish(event T, terminal bool) {
	...
	s.sequence++
	s.latest = event.withSequence(s.sequence)
	s.hasLatest = true
	if terminal {
		s.closed = true
	}
	...
	for id, watcher := range s.watchers {
		select {
		case watcher.events <- s.latest.clone():
			if terminal {
