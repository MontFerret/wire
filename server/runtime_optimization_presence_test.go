	...
	for _, debug := range []bool{false, true} {
		...
		for _, test := range []struct {
			...
			t.Run(map[bool]string{false: "normal/", true: "debug/"}[debug]+test.name, func(t *testing.T) {
				...
				levels := hosted.compileLevels[len(hosted.compileLevels)-2:]
	...
	for _, adapter := range []bool{false, true} {
		for _, debug := range []bool{false, true} {
			for _, outcome := range []string{"success", "invalid level", "callback errors", "callback cancellation", "already cancelled"} {
				name := map[bool]string{false: "Client/", true: "Runtime/"}[adapter] + map[bool]string{false: "normal/", true: "debug/"}[debug] + outcome
				t.Run(name, func(t *testing.T) {
					hosted := &contractRuntime{}
					env := newIntegrationEnv(t, hosted)
					gate := &allocationResponseGate{ClientConnInterface: env.conn, calls: make(map[string]int)}
					var compile func(context.Context, []api.PlanOption) error
					if adapter {
						remote, err := client.NewRuntime(testContext(t), gate)
						if err != nil {
							t.Fatal(err)
						}
						...
						compile = func(ctx context.Context, options []api.PlanOption) error {
							create := remote.Compile
							if debug {
								create = remote.CompileDebug
							}

							plan, err := create(ctx, api.Source{Content: "RETURN 1"}, options...)
							if err != nil {
								return err
							}
